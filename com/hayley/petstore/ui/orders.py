...
from com.hayley.petstore.dao.order_dao import OrderDao
...
class Orders(tk.Tk):
    def __init__(self):
        ...
        self.login_btn = tk.Button(self, text="查看所有订单", command=self.prodslist_btn_onclick)
        ...
        self.listbox1.grid(row=5, column=0, padx='5px', pady='5px', ipadx='5px', ipady='5px')  # 别忘记布局，不然不会显示在窗口上。xww
    ...
    def prodslist_btn_onclick(self):
        ...
        if orders is not None:
            tkinter.messagebox.showinfo("提示", "显示所有的订单喽！")
            name_tmp = []
            for product in orders:
                name_tmp.append(product['orderid'])
